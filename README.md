...
| `POST` | `/api/sessions/{id}/reshuffle` | Optional body: `{"resting_player_ids":["..."]}` for round sessions (recorded on the round; applies to that round only) and `{"exclude_player_ids":["..."]}` for legacy excludes |
...
| `DELETE` | `/api/sessions/{id}/courts/{court_id}` | Remove a court; during an open round its slot is removed and any saved result discarded, then standings are recalculated |
...
- **Multiple courts:** New sessions may configure 1–8 synchronized courts. Singles assigns 2 players per court; doubles assigns 4. Players appear on at most one court per round. If the active roster cannot fill every configured court, remaining courts are returned and displayed as `unused`. Growing the court count during an open round adds a pending pairing from unassigned, non-resting active players and preserves existing pairings and results; it is rejected when too few spare players remain. Removing a court is done with the court-specific endpoint above.
